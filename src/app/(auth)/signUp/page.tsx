"use client"
import { BadgeCheck, Eye, EyeOff, Fingerprint, Loader2Icon, LockIcon, LucideArrowRightFromLine, Mail, ShieldCheck, ShoppingBasket, Store, User } from 'lucide-react'
...
export default function SignUp() {
  ...
  const [ createdUser, setCreatedUser ] = useState<newUser>({
    name: '',
    lastName: '',
    userName: '',
    email: '',
  ...
  const onSubmit = async (e: React.FormEvent): Promise<void> => {
    ...
    setCreatedUser({name: '', lastName: '', userName: '', email: '', password: '', role: 'Cajero'});
  }
  ...
  return (
    <div className='flex flex-row w-full h-full'>
      <Toaster/>
      <div className='flex flex-col sm:py-4 2xl:py-15 px-10 w-full lg:w-1/2 bg-white justify-between '>
        ...
        <div className='flex flex-col w-full max-w-xl items-center justify-center mx-auto'>
          <div className='flex flex-col space-y-2 mb-4'>
            <h1 className='font-extrabold text-3xl lg:text-4xl text-slate-900 tracking-tight'>Registro de Personal</h1>
          ...
          <form onSubmit={onSubmit} className='flex flex-col w-full gap-y-5'>
            <div className='flex flex-col md:flex-row w-full gap-x-4'>
              <div className='w-full md:w-1/2 group'>
                ...
                <div className='relative mt-1'>
                  ...
                  <input type='text' onChange={(e) => setCreatedUser({ ...createdUser, [e.target.name]: e.target.value})} name='name' value={createdUser.name} placeholder='Nombre del Personal' className='w-full bg-slate-50 border border-slate-200 text-slate-900 text-sm rounded-lg p-3 pl-9 focus:ring-2 focus:ring-red-100 focus:border-red-500 block outline-none font-medium placeholder:text-slate-400 transition-all'/>
                </div>
              </div>
              <div className='w-full md:w-1/2 group'>
                ...
                <div className='relative mt-1'>
                  <User className='absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400 group-focus-within:text-red-600 transition-colors'/>
                  <input type='text' onChange={(e) => setCreatedUser({ ...createdUser, [e.target.name]: e.target.value})} name='lastName' value={createdUser.lastName} placeholder='Apellidos del Personal' className='w-full bg-slate-50 border border-slate-200 text-slate-900 text-sm rounded-lg p-3 pl-9 focus:ring-2 focus:ring-red-100 focus:border-red-500 block outline-none font-medium placeholder:text-slate-400 transition-all'/>
                </div>
              </div>
            </div>
            <div className='flex flex-row'>
              <div className='flex flex-col w-full group'>
                ...
                <div className='relative mt-1'>
                  ...
                  <input type='text' onChange={(e) => setCreatedUser({ ...createdUser, [e.target.name]: e.target.value})} name='userName' value={createdUser.userName} placeholder='Cajero_1' className='w-full bg-slate-50 border border-slate-200 text-slate-900 text-sm rounded-lg p-3 pl-9 focus:ring-2 focus:ring-red-100 focus:border-red-500 block outline-none font-medium placeholder:text-slate-400 transition-all'/>
                </div>
              </div>
            </div>
            <div className='flex flex-row'>
              <div className='flex flex-col w-full group'>
                ...
                <div className='relative mt-1'>
                  ...
                  <input type='text' onChange={(e) => setCreatedUser({ ...createdUser, [e.target.name]: e.target.value})} name='email' value={createdUser.email} placeholder='example@example.com' className='w-full bg-slate-50 border border-slate-200 text-slate-900 text-sm rounded-lg p-3 pl-9 focus:ring-2 focus:ring-red-100 focus:border-red-500 block outline-none font-medium placeholder:text-slate-400 transition-all'/>
                </div>
              </div>
            </div>
            <div className='flex flex-row'>
              <div className='flex flex-col w-full group'>
                ...
                <div className='relative mt-1'>
                  ...
                  <input type={showPassword ? 'text' : 'password'} onChange={(e) => setCreatedUser({ ...createdUser, [e.target.name]: e.target.value})} name='password' value={createdUser.password} placeholder='••••••••••••' className='w-full bg-slate-50 border border-slate-200 text-slate-900 text-sm rounded-lg p-3 pl-9 focus:ring-2 focus:ring-red-100 focus:border-red-500 block outline-none font-medium placeholder:text-slate-400 transition-all'/>
                </div>
              </div>
            </div>
            ...
            <button type='submit' className='flex justify-center items-center text-white bg-red-500 py-2 lg:py-4 rounded-2xl w-full uppercase font-bold shadow-lg shadow-red-500/30 hover:shadow-red-500/40 transition-all transform hover:-translate-y-0.5 cursor-pointer'>
              <span className='flex justify-center items-center'>Registrar Cajero{isLoading ? <Loader2Icon className='w-5 h-5 mt-0.5 ms-1 animate-spin'/> : <LucideArrowRightFromLine className='w-5 h-5 mt-0.5 ms-1'/>}</span>
            </button>
            <div className='flex justify-center text-sm text-slate-400 mt-2'>
              <span>¿Ya tienes cuenta? 
                <Link href="/login" className='text-red-500 font-bold ms-2 cursor-pointer hover:underline'>
                  Iniciar Sesión
                </Link>
              </span>
            </div>
        ...
        <div className='flex w-full justify-between items-end'>
          <span className='text-sm text-slate-400'>© 2026 Minimarket Daniela.</span>
           <div className='flex flex-col items-end'>
             <span className='text-xs text-slate-400 mb-1'>¿Problemas de acceso?</span>
             <button className='text-sm font-bold text-red-500 hover:underline flex items-center gap-1'>
      ...
      <div className='hidden lg:block lg:w-1/2 relative bg-slate-900 overflow-hidden'>
        {backgroundImages.map((img, index) => (
          <div key={index} className={`absolute inset-0 transition-opacity duration-1000 ease-in-out ${currentImage === index ? 'opacity-100' : 'opacity-0'}`}>
            <Image
              ...
              fill
              className={`object-cover transition-transform duration-9000 ease-in-out ${currentImage === index ? 'scale-110' : 'scale-105'}`}/>
          </div>
        ))}
        ...
        <div className='absolute inset-0 flex flex-col justify-end items-start z-40 ms-24 space-y-14 mb-8'>
          <div className='space-y-3 max-w-2xl'>
            <div className='bg-white/10 backdrop-blur-md border border-white/20 w-fit text-white text-xs px-4 py-1.5 rounded-full tracking-wider mb-2 uppercase font-bold flex items-center gap-2'>
          ...
          <div className='flex gap-2'>
            {backgroundImages.map((img, index) => (
              <button key={index} onClick={() => setCurrentImage(index)} className={`h-1.5 cursor-pointer rounded-full transition-all duration-300 ${currentImage === index ? 'w-8 bg-red-500' : 'w-2 bg-white/30'}`}></button>
            ))}
          </div>
        </div>
      </div>
